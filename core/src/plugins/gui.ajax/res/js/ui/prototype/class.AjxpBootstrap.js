...
Class.create("AjxpBootstrap", {
	...
	refreshContextVariablesAndInit: function(connexion){
        ...
        var masterClassLoaded = function(){
            ...
            window.pydio = window.ajaxplorer = new Pydio(ProtoCompat.hash2map(this.parameters));
	...
	detectBaseParameters : function(){
		$$('script').each(function(scriptTag){
			if(scriptTag.src.match("/js/ajaxplorer_boot") || scriptTag.src.match("/js/ui/prototype/class.AjxpBootstrap.js")){
				if(scriptTag.src.match("/js/ajaxplorer_boot")){
					this.parameters.set("debugMode", false);
				}else{
					this.parameters.set("debugMode", true);
				}
                var src = scriptTag.src.replace('/js/ui/prototype/class.AjxpBootstrap.js','').replace('/js/ajaxplorer_boot.js', '').replace('/js/ajaxplorer_boot_protolegacy.js', '');
	...
	insertBasicSkeleton : function(desktopNode){
        ...
        if(document.viewport.getWidth() < 500){
            ...
            elem.update('Pydio, open source file sharing - <a target="_blank" style="color: #7a7a7a;" href="https://pydio.com/">https://pydio.com/</a>');
        }
